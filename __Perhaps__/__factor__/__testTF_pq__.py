...
def dist(X, Y):
    ...
    for i in range(1, m + 1):
        ...
        for j in range(1, n + 1):
            pq = PQ_GramWrapper._get_pq_edit_distance(X[i-1], Y[j-1], 2, 3)
            if pq <= 0.9:           #(ケース2)
                cost = 0                        #(ケース2)
            else:
                cost = 1                        #(ケース3c)
...
def main():
    ...
    for test_case in test_cases:
        ...
        for tt in test["children"]:
            ...
            test_sequence.append(astCommand)
        ...
        for case in tqdm.tqdm(cases):
            ...
            for astCommand in case["content"]["children"]:
                ...
                case_sequence.append(astCommand)
            edit_distances.append(
                {
                    "caseId": case["caseId"],
                    "answer": case["answer"],
                    "distance": case["distance"],
                    "pq_distance": dist(test_sequence, case_sequence)/max(len(test_sequence), len(case_sequence))*1.00
                }
            )

        edit_distances = sorted(edit_distances, key=lambda x:x["pq_distance"])
