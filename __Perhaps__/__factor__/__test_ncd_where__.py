...
def main():
    ...
    # test_case = "casperjs:0"
    # test_case = "100644861884e21cc1e1aa878b21042b810f04f4:2"
    # test_case = "4ee640d195f408deedaa8080d7e4c78438e1b9e1:0" #aptGetUpdatePrecedesInstall
    # test_case = "chinadns:0"
    test_case = "mariadb:0"
    test_obj = {
        "type": "ROOT",
        "children": []
    ...
    test_ncd.append(test_ncd[-1])
    # test_ncd[1], test_ncd[2] = test_ncd[2], test_ncd[1]
    ...
    for dumped_id, dumped_ast_commands in tqdm.tqdm(dumped_ast_commands_per_run_instruction_dictionaly.items()):
        ...
        ncd_dist, where = dist(test_ncd, sample_ncd)
        edit_distances.append(
            {
                "dumpedId": dumped_id,
                "astCommands": sample_obj,
                "ncd_distance": ncd_dist/max(len(test_ncd), len(sample_ncd))*1.00,
                "where": where,
    ...
    for output_distance in output_distances:
        ...
        for edit_distance in edit_distances:
            ...
            print(edit_distance["dumpedId"].ljust(30), edit_distance["ncd_distance"], edit_distance["simple_distance"], where)
            count += 1
